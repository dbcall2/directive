...
class TestLegacyDetection:
    ...
    def _write_current_generated_spec(self, project_root):
        vbrief_dir = project_root / "vbrief"
        vbrief_dir.mkdir()
        for folder in ("proposed", "pending", "active", "completed", "cancelled"):
            (vbrief_dir / folder).mkdir()
    ...
    def test_ignores_current_generated_specification(self, tmp_path, deft_run_module):
        self._write_current_generated_spec(tmp_path)
        assert deft_run_module._detect_pre_cutover_legacy(tmp_path) == []
