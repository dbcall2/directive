...
- `task scope:promote -- xbrief/proposed/<file>.xbrief.json` -- move proposed work to `pending/` and set status to `pending`. When `plan.acceptance` is absent, empty-with-none_stated, or command-only-without-clauses, runs #3323 clause derivation and stamps `clauses[]` (#3360). A 0-clause no-op refuses (#4768): supply list items, `test:` lines, or `acceptance:` lines. The intake floor stays legal before that step and is not committed.
...
- `task scope:activate -- xbrief/pending/<file>.xbrief.json` -- move accepted work to `active/` and set status to `running`. Same #3323 derivation hook as promote. When that derivation applies false and leaves no clauses, activate refuses and does not commit the intake floor (#4768). Recovery is list items, `test:` lines, or `acceptance:` lines (#4374). #3334 still refuses an invalid `plan.acceptance` when clauses are already present. Headless: records `chosen_reading`, never blocks on a question.
...
- `task scope:bind-clause -- <active-brief> --clause N[,M] --path <file_scope-member>` -- explicit bind of selected pathless derived clauses onto an approved `file_scope` matchAny path before stamp (#4986). Empty `file_scope` is a silent no-op. Does not lift a path from issue or comment text (#3835).
- `task scope:stamp-evidence -- <brief>` -- evidence-only writer: records kind:test via stampMatchAnyFileEvidence onto clauses that already have a non-blank in-scope `artifact_path` (#4840 / #4986). No --pr / --merge-commit / disposition argv.
...
- Allow confirmed #1019 preflight-gh opt-out: `deft policy:allow-destructive-gh-verbs -- --confirm`
- Re-enforce #1019: `deft policy:enforce-destructive-gh-verbs`
...
- `task verify:completed-write-guard` -- fail closed when a newly added `xbrief/completed/` (or `vbrief/completed/`) artifact was not written by `scope:complete` / `scope:fail` (#3679), or when a change set deletes or renames away an `xbrief/active/` brief with no paired stamped destination (#3766). Same name-status parser covers A, R, and D. Terminal moves pass as `R active->completed` or `D active` plus `A completed` when the destination has a runTransition stamp. A `cancelled/` destination pairs only when it carries a `lifecycleWrite` cancel stamp. A `proposed/` destination pairs leftover park of still-open work when `plan.status` is `proposed` or `draft` and there is no cancel stamp. Parking an open issue off `active/` is `proposed/`, not `cancelled/`. Current live GitHub state must confirm closure before the guard refuses that park; stale cached closure cannot block reopened work. Closed work needs `completed/` or `cancelled/` plus evidence. Delete-plus-add pairing requires pairingKey, dest `plan.title`, and origin issue refs to match the recovered source, plus a valid `lifecycleWrite` complete or cancel stamp (#4784). Item titles and narratives are not pairing identity. A copied stamp with a different title or origin still refuses. Lone-D untracking cleanup halts and names `scope:complete` or leaving the brief untracked. Artifacts larger than 1 MiB fail closed before read. Historical corpus is advisory via doctor. Remediation names the leftover land PR after a scope-provenance strip (#3476). `scope:complete` can stamp a brief already in `completed/`. Does not change `verify:completed-tracked`.
...
- `task verify:forward-coverage` / `deft verify:forward-coverage` -- fail-closed new-source-file existence (#1310) plus warn-first diff coverage of added/modified branches (#3514). Intersects `coverage/coverage-final.json` with the diff against a 90% per-change branch threshold. That 90% is coverage of new code; the project vitest floor (75) is a collapse detector for the aggregate -- they are not interchangeable. Missing coverage reports skip the diff half (existence still runs). Pass `--enforce` to fail closed on uncovered changed branches; `--staged` for pre-commit. A paired file does not count when its assertions only say an export is a function, or when it reads the paired source as text and expects strings (#4543).
...
- ! **Identity-only + product mutation (#4544):** on the session-routing else-branch (`else main → USER → PROJECT-DEFINITION; ~ sync` then Mutation → `deft session:start`), when live PROJECT-DEFINITION is identity-only (empty `plan.items` and no convention-valid lifecycle scope) AND the live request has product mutation intent, join existing setup Phase 3 Starting-new / Rapid BEFORE that mutation `session:start`. After Phase 3 confirmation: write confirmed `plan.narratives.Overview` (Light narrative merge or an explicit Overview patch before render; Rapid Strategy Gate STOP must not skip that write) plus one `xbrief/proposed/` draft that carries the live request's requirement list through Rapid Step 3 and #3267 capture. Process-only stays an explicit exit. ⊗ Add `session:start --prompt`. ⊗ Auto-write `xbrief/active/` — promote and activate stay the existing verbs. Unmanaged header one-liner is placeholder-only compare-and-set from user-confirmed Overview (not a prompt interpolator; not identity source of truth). Handback reports the actual lifecycle outcome (`proposed <path>`, Process-only, or product-write-with-no-brief as a gate anomaly). Resolve the rapid compose pointer through `contentRoot` / the consumer deposit path. Under rapid, if this session changed product files and no brief exists, `deft check` reports the verify:ac soft-skip as a named warning.
- ? Explicit read-only alignment only: `deft session:start --read-only` (no ritual-state write). Task: `task session:start -- --read-only`.
- ? Tracked docs/requirements without a story xBRIEF: `deft session:start --posture=requirements` (occupancy claimed; no gated ritual). Token is `requirements`; `docs` remains assist/scratch. Unknown --posture tokens refuse and print the closed set (owner: packages/core/src/session/posture.ts). Forge-only (#4020) is no-ingest with dest worktree permitted -- not a tree-write posture.
...
- ! **Stale-ritual recovery matches ceremony eligibility (#4290):** occupancy preview (`write: false`) evaluates the same claim/heartbeat/steal eligibility as persist, including the live-sibling primary fence. Persistence re-reads the sibling leases under the occupancy lock so a lease that arrives after preview still refuses without writing. Stale-ritual denials name `session:ready` / `session:start --rearm` / cold `session:start` only when that ceremony would admit this actor; a restricted primary names the existing `--primary-claim-exception=operator-default-branch` argv or a linked-worktree claim with the same actor identity. A member write grant does not authorize an owner claim. Tree/HEAD continuity does not establish operator identity. Dedicated Write/Edit and recognized in-repository shell forms are gated; unrecognized shell forms remain outside that coverage.
...
| **Requirements** | `session:start --posture=requirements`; occupancy claimed; no gated ritual; no story xBRIEF | Tracked docs/specs/proposed xBRIEF class after occupancy (`write-requirements-ready`). Token is `requirements` -- `docs` stays assist. | Not a spawn class |
...
- ! **Tracked / source still hard:** writes to product paths (`src/`, `packages/`, `content/`, app source, tracked `docs/` / `overview/`, …) under assist, read-only, or mutation still require mutation ceremony + existing write/scope gates. Requirements posture (`session:start --posture=requirements`) is the tracked-docs exception: occupancy first, then docs/specs/proposed xBRIEF writes (`write-requirements-ready`). Instruction-surface files (AGENTS.md, main.md, SKILL.md, contracts) stay denied. Scratch docs remain the assist route under .deft-scratch/. Labeling a change "research" does **not** bypass them. If the operator insists on a tracked path for notes, reclassify as mutation or obtain explicit override + normal gates; prefer redirect to `.deft-scratch/overview/` instead.
- ! **Requirements posture (#4444):** declare via trusted `deft session:start --posture=requirements` (writes gitignored `.deft/session-posture.json`; `DEFT_SESSION_POSTURE` still wins when set). Occupancy runs before any allow. Path class uses `evaluateRuntimeAuthorityPath` (default allow: docs/**, specs/**, README.md, xbrief/proposed/**; deny instruction surface at any depth, including nested SKILL.md/AGENTS.md/main.md). Symlink components on an otherwise-allowed path are refused. Out-of-class writes refuse with a mutation upgrade path. Do not reuse `write-assist-scratch-ready`. Payload posture fields are not a trusted producer.
...
**Ordered plan verbs:** `deft plan-sequence:set`, `deft plan-sequence:current`, `deft plan-sequence:advance`, `deft plan-sequence:clear`, `deft verify:plan-sequence --target-kind <kind> --target <id>`. `plan-sequence:status` is not a verb. When the sequence is exhausted, stop until the operator names a new target or explicitly asks for queue/backlog selection ("what's the queue?", "build a cohort"). When the current entry is still pending but its Directive origin already sits in `xbrief/completed/` or `cancelled/`, `plan-sequence:current` and `verify:plan-sequence` fail closed with `terminal-lifecycle` and ask the operator — do not treat that as permission to pick the next id or to run `plan-sequence:advance` unattended (#4129). Do not reuse triage queue `continuationNumbers` / `continuationOrder` for ordered-plan state.
...
- **Ordered-plan precedence (#2402):** when `.deft/plan-sequence.json` is active, bare "what's next?" / "next PR" / "proceed" bind to the current sequence entry via `deft plan-sequence:current` — they do **not** authorize `triage:queue` or adjacent backlog picks. Use `deft verify:plan-sequence --target-kind <kind> --target <id>` before opening a PR/branch/story/sub-agent. Sequence exhaustion fails closed until the operator names a new target or explicitly asks for queue/backlog selection ("what's the queue?", "build a cohort"). A pending current entry whose origin is already in `completed/` or `cancelled/` also fails closed (`terminal-lifecycle`); stop and ask rather than advancing (#4129). Set a sequence with `deft plan-sequence:set --file <json>`; advance with `deft plan-sequence:advance`; clear with `deft plan-sequence:clear`. Do not reuse triage queue `continuationNumbers` / `continuationOrder` for this state.
...
- **Stale ingest-ready scan (#4970).** `task scm:issue:design-critique-stale-ready -- [--repo OWNER/NAME]` (`deft scm issue design-critique-stale-ready`) is read-only. It enumerates open `design-critique:ingest-ready` issues and reports standing chips whose live completed-arc record no longer admits ingest. Missing Stop 1 pain coverage prints the later-arc recovery and the existing chip command, which keeps the scanned repository. Other reasons keep evaluator detail. The scan writes nothing.
