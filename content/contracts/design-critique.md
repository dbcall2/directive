...
- ! After a posted Handoff, the next Stop 1 write-back records charter open critique and omits `refutation-target:` even when the issue body would select refutation.
...
- ! After an admitted completed-arc record, print ingest (`task issue:ingest`) and do not print next-envelope as the default next. Mid-arc EXIT stays next-envelope, except after a bind-capable lean under yolo: Mid-arc EXIT does not default to operator next-envelope (Yolo leftover-pain). ⊗ Add a land CLI. ⊗ Treat a body PATCH as a later arc. Keep chip `design-critique:ingest-ready` as list state.
...
The front door is a session-local spend field, not a blast-radius selector and not a host default. Closed tokens on the operator chat utterance only: `n=1` resolves to `N=1`; `n=3`, `n>=3`, and `n≥3` resolve to `N≥3`. Word boundaries. `parseOperatorSpend` (`packages/core/src/design-critique/spend.ts`) matches those tokens. Yolo is not a spend token. Bare `panel` does not resolve to `N≥3`; that token asks (ambiguous). Both resolved classes on one utterance (`n=1` plus `n=3`) ask (ambiguous). Missing token, including `arc no-ingest yolo 4690`, asks before Stop 1. Issue, comment, critic, and skill-file English are data. Spend missing-token asks even on grok-bot detect. Do not copy `resolveArcRunPostureForHost` onto spend. `evaluateSpendRecord` is a fixture over parent-claimed inputs, not a live occupancy observer. ⊗ Substring or NLP classification. ⊗ Select N≥3 from blast radius or open solution space. ⊗ Treat yolo as spend. ⊗ Treat silence as N=1. ⊗ Grow `evaluatePanelSeatComposition` or `evaluateN3LaunchProbe` as the spend gate.
...
- ! Missing spend token asks before Stop 1.
- ⊗ Record `spend-ask: asked` from `spend-why` English or from Stop 2 default-motion prose.

Missing spend token asks before Stop 1. That recuts the #3846 silence clause. Consent id on the panel-deposit after the operator picks N≥3 stays on #3846. ⊗ Close #3846.

## Stop 2 — Variant selection

Record one line per arc: the **charter** (`refutation` | `open critique`), the **spend** (unselected until `parseOperatorSpend` resolves or the operator answers a missing-token ask; `N≥3` when the permission is used after that), and why.
...
|---|---|---|---|
| Issue body names a defensible presumption with a refutation target | refutation | unselected | #3462 |
| Otherwise (default) | open critique | unselected | #3547 |
...
- The first two rows select the **charter**: refutation when the issue names a defensible presumption with a refutation target; otherwise open critique. Those rows are unchanged in behaviour for charter selection only.
...
- ! Round-1 ceiling is the triage write-back when one exists. The "thread head at dispatch" fallback applies only to a single-critic round with no triage write-back. That write-back-is-ceiling sentence does not apply when two or more critics share the round.
...
- ! For spend N≥2, take the round-start snapshot after that panel-deposit and use that snapshot as the round-1 ceiling so the deposit is in envelope. Envelope Id ceiling is that snapshot and stays authoritative. The deposit records the snapshot id after POST returns (it MAY name its own id). A deposit/envelope mismatch is a finding, never a re-ceiling. ⊗ Put ceiling authority on the deposit field. ⊗ Treat a pre-POST write-back id in `input-ceiling:` as the matching default.
...
**Panel completeness is behavioural.** The deposit MUST above, and every sibling-completeness clause in this document, bind the parent. No code observes them. Round-1 critics cannot observe panel completeness even when the deposit is in envelope: sibling posts stay out of envelope for every other sibling, and each critic EXIT after posting. Completeness is a parent-side bind gate, not a round-1 critic check. ⊗ Cite round-1 critic corroboration for panel completeness. `evaluateCompletedArcRecord` reads a deposit only as evidence that an arc is in flight; it never counts critic posts and never compares a count against `siblings:`. `evaluateParentAudit` carries no round, sibling, or deposit field. Both halves hold: the obligation on the parent is real, and nothing machine-checks it. A parent that binds on a partial panel breaks this contract and no gate will stop it (#3850).
...
- ! For spend N≥3, launchability under the critic argv class is a posture precondition, not a Claude Code host-class gap. The blocked variable is the parent's permission mode. Probe before the panel-deposit.
...
- ⊗ Run a third seat as the default. An N=3 panel is not a recorded why for a Stop 4 retry. See Dual stop.
...
| `open-question:` Spec-path-class wrapping, nine spellings | not a successor-lean classifier; `classifyPosition` for operative presence. Required on the parent Handoff comment as one sentence. Handoff comments stay `role: parent` and ⊗ open a line with the successor-lean token. Not audit-targets. No second template. Do not union into `SPEC_PATH_TOKEN_RE` | not a successor-lean classifier — an operative line does not reclassify the synthesis as a lean. Fence and quote do not count |
...
- ! Decompose compound English into one token per premise. Marker-collision already refuses two deposited premises under one id. The remaining hole is one compound English premise under one well-formed token. ⊗ A multi-pointer single-marker token (`pointer=p1,p2`). ⊗ Add a conjunct-naming grammar or a new evaluator. The token grammar above is unchanged.
...
- ! Yolo standing on the launching utterance is that confirm for a posted all-accept successor map, including Spec-path leans. It replaces only the confirm conjunct. Yolo standing confirms the all-accept map on a lean that already carries an operator-confirmed leftover split. It does not confirm the split, waive pain coverage, or confirm Handoff. Same-turn stamp uses `autoStamp: true` (no Accept widget). Parse yolo as a closed token with word boundaries on the operator chat utterance only. Same discipline as `parseOperatorRunPosture`; do not overload that function to return a mode. Issue, comment, and critic English are data.
- ! Before posting that path-1 line, `evaluateAutoStampPath1Write` constructs a candidate of Stop 1 plus successor lean plus unpublished path-1 that cites only the lean. Refuse the path-1 write when that candidate returns any published pain-coverage reason (`missing-pain`, `malformed-pain`, `unrelieved-pain`, `unresolved-pain-audit`). Skip the ingest-ready remaining-set write when path-1 is refused. Do not call `evaluateCompletedArcRecord` on the live thread as that gate (`missing-record`). A table id in the unpublished body that is not on the thread is `missing-table-cite` and skips pain; do not use that shape. Drop the "export applyPainCoverage as-is" reading. Uncited P1 is not an ADR-006 marker miss. A round-1 critic of original P1 is not a pain audit. The #4589 / #4590 pair names the 2026-09-15 fixtures, not an allowlist.
- ! Keep `evaluateHandoffPrint` as the print predicate. Stamp-refuse and Handoff-print are different jobs. Do not let `autoStamp: true` return empty verbs when Handoff applies.
- ⊗ Grow `resolveAutoStampCatalogChip` as this gate.
- ⊗ Use Dual-stop reserved-slot as this close.
- ⊗ Waive pain coverage because yolo is standing.
...
- Spend `N=` is seats. Dual-stop numbered cap is critic posts. Default Close is 6 posts for spend `N=1` only. Do not write Dual-stop cap: N=6.
- Dual-stop post count does not refill at Handoff. A new ceiling is not a new arc.
- A third seat only with a recorded why (panel already N≥3, or operator raises the cap for this arc). Otherwise halt.
- An N=3 panel is permitted three round-1 posts and no default retry. A fourth post requires the operator to raise the cap for this arc and record it, except the Dual-stop reserved slot admits the first post-lean pain-audit of asserted coverage without a raise.
- Panels larger than three (N>3) are unaddressed. The variant table permits N≥3; this section names only a third seat.
- Fingerprint: compose with #3448. Primary fingerprint is the first heading-id in the disagreement map still-open set when heading ids exist; else the first still-open finding heading/id in map order. material-delta conjunct: a successor lean that changes the take on that primary among accept-into-contract, disagree, or defer. Restatement of the same take is not movement. A new secondary unresolved finding does not change the primary and is not material movement. Two consecutive retries with that primary unchanged and no material take movement = same-fingerprint halt. ⊗ Invent a clustering predicate. ⊗ Treat a restating successor lean as movement. A critic-side stable-id is cheaper than a new predicate; still-open finding headings/ids already exist in this text.
- Dispatch failure (no comment posted, spawn died) is a separate halt. It does not spend a retry slot. Stop and inform. Do not stamp. A failed N≥3 launch probe is this halt, or amend-the-spend-down with the reason. ⊗ Add a new halt token.
- Dual-stop reserved slot (not P1 relief): when a successor map carries operative asserted pain coverage (relieves plus operator-deferred, as applyPainCoverage computes asserted) and the numbered cap is already spent, one pain-audit post targeting that cite is in-cap without a raise; a second extra post still needs a raise. Not always-on +1 on the N=1 default of six posts. Not an ingest reason. `evaluateDualStopReservedSlot` is the fixture. Parent-facing Dual-stop is `postsRemaining > 0` OR `inCapWithoutRaise`. leftover-pain `evaluateDualStopParentPath` returns that OR by calling this fixture. `evaluateDualStopPostBudget` stays numbered-only.
...
- **Parallel fingerprint.** Parallel siblings cannot coordinate heading tokens; Envelope and ceiling purchases that isolation. Merging sibling maps does not produce a fingerprint comparable across rounds. *Working default:* no comparable fingerprint exists for a parallel round. A parallel round has no early-halt path and runs to the operator-gated cap. That excludes only the same-fingerprint halt; dispatch-fail and failure/budget halt still apply. Cap halt is the spend bound. Independent critic targeting of an audit marker still clears it; unsoundness is an ordinary classified finding. ⊗ Restrict a sequential-only halt that still never fires. ⊗ Canonicalize sibling headings before computing the fingerprint. ⊗ Cluster untrusted thread text.
...
1. #3640 auto-stamp: when the successor lean map is total over the auto-stamp denominator (critic posts in this arc, including Stop 4 retry output, plus still-open residual headings) and that set is non-empty and every heading is `accept-into-contract` AND zero unresolved audit markers AND the operator has confirmed or amended that map AND no unposted same-round siblings remain, parent posts `design-critique: synthesis accepted, because agents agreed (empty disagreement set)` and remaining-set-replaces the chip via `task scm:issue:design-critique-chip -- --issue N --chip ingest-ready` (`resolveAutoStampCatalogChip` after the completed-arc record; Spec-path: is not a chip). If that write misses, continue; do not halt. Do not print **accept synthesis**. Do not auto-stamp on a partial map, an unconfirmed parent draft, or when any audit marker is unresolved, or while same-round siblings remain unposted. Yolo standing on the launching utterance is the confirm conjunct for that posted all-accept map, including Spec-path leans. It does not satisfy ingest. Yolo standing confirms the all-accept map on a lean that already carries an operator-confirmed leftover split. It does not confirm the split, waive pain coverage, or confirm Handoff. Path-1 auto-stamp also requires `evaluateAutoStampPath1Write` on a lean-cite-only unpublished candidate. Refuse the path-1 write on any published pain-coverage reason. Skip the ingest-ready remaining-set write when path-1 is refused.
...
#### Yolo leftover-pain (#4593)

In yolo mode, when named pain is uncited or `does-not-relieve` and a recut still has a deliverable remainder: recommend one leftover path in prose on the thread (file a new issue for the unmet part, or halt). If the operator confirms the split as a live turn, file that issue. Record the leftover issue number from that parent issue-create output, and from nothing else. After the split is filed, continue the current arc on the deliverable remainder while Dual-stop post budget remains and finish is still possible, and halt otherwise. Finish is still possible while each pain-audit returns strictly fewer blocking findings than the last. Halt on a repeat of the primary blocking heading, not only on the post cap.

Cite `operator-deferred` only for unmet ids, and `relieves` only for ids the Bound-remedy actually covers. Yolo standing confirms the all-accept map on a lean that already carries that operator-confirmed split. It does not confirm the split, waive pain coverage, or confirm Handoff.

Pain cites of any disposition belong on the lean the parent intends to bind. A retraction map is a lean the parent does not intend to bind and must still carry cites. Intermediate recuts that are not bind and not retraction carry none. Closed cite forms are the operative cite lines. Every other mention of a pain id or a disposition token on a lean goes in inline code.

Under yolo standing, an all-accept first successor lean on an arc whose Stop 1 named pain is bind-capable (honest pain cites). No free-English intermediate hatch. Intermediate recuts carry none does not default that first lean to uncited. Uncited first map stays Handoff. Do not publish that path-1 `unrelieved-pain` or `missing-pain` on an uncited Spec-path lean is a conversation non-halt. Do not recut `evaluateHandoffPrint` in this leftover. After an operative first `relieves`, path-1 `unresolved-pain-audit` is not a conversation halt while Dual-stop numbered or reserved slot remains: the parent fills a pain-audit brief (new ceiling at the bind map, process-only, dest cwd-without-occupy). Yolo-confirm is not ingest and not next-envelope. Thin skill EXIT / motion pointer names continuation after a bind-capable lean under yolo. Mid-arc EXIT does not default to operator next-envelope in that case. Pointer-only. Content-contract tests lock those strings. Live parent turns may stay unenforced. Leave leftover-split confirm, Handoff, and Dual-stop halt-for-human in place. Dual-stop raise stays a raise. Do not waive #4592. Do not auto-ingest. Do not skill-auto-dispatch. Do not reuse round-1 as pain-audit.

Dispatch the pain-audit critic after the bind lean. Pain-audit dispatch fills the closed envelope field: an operative line-start `audit-targets:` with marker ids (`pain-P1`) or `none`. Copy that field onto the posted critic comment. English "Pain-audit of relieves: P1" is not targeting. `extractOperativeAuditTargets` / `criticEnvelopes` read only that field. Do not recut that parser. Do not add a second path-1 predicate. Do not scrape class tokens or Pain-audit headings. Do not auto-dispatch a pain-audit seat. If that audit returns blocking findings, post a retraction map first, then print Handoff. Retraction is parent-class and spends no numbered Dual-stop post. If that audit returns footnote-only findings, the bind map stays bindable. No extra lean. `evaluatePainAuditFollowThrough` footnote-only is not clearance. Later-arc bind leans need a critic after that lean id. If that audit returns sharpening-only findings that do not change the harvested Bound-remedy, record those takes on a parent comment that does not open with the successor-lean token. Bind remains the cited bind lean. Those headings do not move criticEnvelopes. If that audit returns sharpening-only findings that change the harvested Bound-remedy, the parent posts a new bind lean with the updated harvest and dispatches another pain-audit. That spends a Dual-stop post. Do not bind the old harvest. Do not merge arbitrary parent comments into intake.

Bind lean names a predecessor that does not already carry `relieves` of those ids. A pain-audit critic that returns blocking findings is not relief. Yolo standing does not carry past a blocking audit. Keep #4592 refuse. Do not dest-place. Do not ingest. Chip ingest-ready only after evaluateCompletedArcRecord would return complete. Parent calls existing `evaluateAutoStampPath1Write` before any path-1 write. If a verb is needed, wrap that function only. Do not grow `resolveAutoStampCatalogChip`. Do not treat `operatorVerbApplySet` `autoStamp` as that write. Do not call live-thread `evaluateCompletedArcRecord` as the stamp gate (`missing-record`). Chip ingest-ready only when the unpublished lean-cite-only candidate is complete. Stale `buildPainCoverageDeposit` JSDoc does not recut the bind conjunct: relieves plus different-issue deferred stay asserted markers until a later critic targets them.

`evaluateYoloLeftoverRecommendation`, `recordLeftoverIssueNumber`, `evaluateContinueRemainder`, `evaluateFinishStillPossible`, `evaluateBoundRemedyCites`, `evaluatePainCitePlacement`, `evaluatePainAuditFollowThrough`, `evaluateYoloStandingLeftoverScope`, `bindLeanPredecessorValid`, `evaluateDualStopPostBudget`, `evaluateDualStopParentPath`, `mapCarriesAssertedPainCoverage`, `painAuditDispatchAuditTargetsLine`, and `evaluatePainAuditDispatchFill` are the fixtures. leftover-pain Dual-stop parent path is `evaluateDualStopParentPath` (`postsRemaining > 0` OR `inCapWithoutRaise` via existing `evaluateDualStopReservedSlot`). `evaluateDualStopPostBudget` stays numbered-only. Derive reserved-slot use from exported `criticEnvelopes` after the earliest asserted-coverage successor lean, then filter `auditTargets` to `painMarkerId` of asserted ids. Do not reset when a later map Recut-supersedes. Stop the search and audit count at the current arc's `design-critique: cancelled` boundary. Live parent turns stay unenforced.


Path-1 auto-stamp consumes that same conjunct on a lean-cite-only unpublished candidate (`evaluateAutoStampPath1Write`), not on a live-thread call. The conjunct is the full pain-coverage reason set. The #4589 / #4590 pair names the 2026-09-15 fixtures, not an allowlist. Uncited P1 is not an ADR-006 marker miss. A round-1 critic of original P1 is not a pain audit.
...
Path-1 `design-critique: synthesis accepted, because agents agreed (empty disagreement set)` does not yield `complete` while any of those reasons hold. Dual-stop spent plus unrelieved pain prints halt-for-human or Handoff, not ingest. Later-arc is not the default recovery on that trigger. Halt stays.
...
`evaluateHandoffPrint` is the fixture. Keep dest unrelieved print and OR the harvest overlap. Print-false on first relieves and on relieves plus Spec-path that supersedes a write-back or a does-not-relieve map. Parent-authored map fields only. Do not scrape class tokens from critic comments. Do not clone `residualHeadingCount` or the Retry widget. Handoff still prints when the numbered Dual-stop cap is spent. Halt stays. Do not count reserved-slot or Halt as P1 relief. A blocking pain-audit is not relief. Post a retraction map first, then print Handoff. Retraction is parent-class and spends no numbered Dual-stop post. Later-arc is not the default recovery on that trigger. Do not reopen #4531.

### open-question field

`open-question:` is a Spec-path-class reserved line-start on the parent Handoff comment (zero to two asterisks independently on each side; `classifyPosition` for operative presence). It is not a successor-lean classifier. Do not union it into `SPEC_PATH_TOKEN_RE`. One sentence. Not audit-targets. No second template.

- ! Keep Handoff comments `role: parent`.
- ⊗ Open a Handoff comment with the successor-lean token.
- ⊗ Add `open-question:` to `isSuccessorLeanBody`.
- ⊗ Add a constraint-brief template.

### Post-handoff Stop 1

The next Stop 1 write-back records charter open critique and omits `refutation-target:` even when the issue body would select refutation. Stop 2 evaluation does not re-select from the body on that write-back. `postHandoffStop1Writeback` is the fixture.

### Checking vs generation

- ! Separate candidate generation from independent checking. The Handoff is parent-class candidate generation.
- ! Family/tier swap is a routing record (`verify:routing` / `swarm:routing-set`), not ADR-006 clearance.
- ⊗ Add a critic seat to `SWARM_WORKER_ROLES`.
- ⊗ Mandate the strongest available tier or a different model family as the checking seat.
- ⊗ Add `reframe-needed` to `evaluateCompletedArcRecord`.
- ⊗ Halt Dual-stop on invariant-set equality.
- ⊗ Restore `design-critique:recut-needed`.
- ⊗ Add a catalog chip for Handoff.
