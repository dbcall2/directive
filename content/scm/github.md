...
- ! Machine observer: `task verify:review-monitor -- --pr <N> --merge-path-arm` with `--live-wait` and/or `--explicit-finish` attestations (optional `--sticky-lease`; Tier 1 live-wait is bound to gate lease evidence for that PR). Lease-only fails closed. Core helper: `evaluateMergePathArm` in `@deftai/directive-core/pr-watch`.
...
**Release census (#3969 / #4917):** `task verify:consumer-hard-stops` blocks a cut only for open `blocks-release-tag`. `adoption-blocker` is the punch list and does not block a tag. The BLOCKER title flare does not by itself block a cut. Issue bodies are not read into the verdict.
