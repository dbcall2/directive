...
export const READY_REQUIRES_PARALLEL_SAFE =
  "readiness=ready requires parallel_safe=true. This story is not eligible for concurrent swarm:launch; dispatch non-concurrent work via the interactive swarm-skill solo-worker path (that path is not gated by swarm:readiness exiting 0; exit 0 gates concurrent workers only; see #3669). N=1 origin per PR unless a closed one-PR-unit grant exists; serialize N PRs, do not combine closes.";
