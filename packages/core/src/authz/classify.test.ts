...
describe("classifyShellAuthzOps (#2944)", () => {
  ...
  it("classifies residual dest-form plants after #3529 as settings (#3545)", () => {
    ...
    expect(classifyShellAuthzOps("find -exec make DESTDIR=.deft/authz/grants install")).toContain(
      "settings",
    );
    expect(
      classifyShellAuthzOps("find . -exec make DESTDIR=.deft/authz/grants install ;"),
    ).toContain("settings");
    expect(
      classifyShellAuthzOps("find . -exec make DESTDIR=.deft/authz/grants install {} +"),
    ).toContain("settings");
