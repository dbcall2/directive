...
function routeNamespaceVerb(ns: string, verb: string, rest: string[]): RoutedArgv | null {
  ...
  if (ns === "scope") {
    ...
    if (verb === "record-intent-constraint") {
      return { kind: "dispatch", argv: ["scope-record-intent-constraint", ...rest] };
    }
  }
  ...
  if (ns === "scm" && verb === "issue:design-critique-stale-ready") {
    return { kind: "dispatch", argv: ["scm", "issue", "design-critique-stale-ready", ...rest] };
  }
