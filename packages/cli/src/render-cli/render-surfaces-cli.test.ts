...
describe("deft-ts spec-render", () => {
  ...
  it("keeps --project-root spec-render compact by default (#1566)", () => {
    const root = mkdtempSync(join(tmpdir(), "deft-cli-spec-render-compact-"));
    temps.push(root);
    const xbrief = join(root, "xbrief");
    mkdirSync(join(xbrief, "completed"), { recursive: true });
    writeFileSync(
      join(xbrief, "specification.xbrief.json"),
      JSON.stringify({
        vBRIEFInfo: { version: "0.6" },
        plan: {
          title: "Compact Spec",
    ...
    writeFileSync(
      join(xbrief, "completed", "2026-01-01-archive.xbrief.json"),
      JSON.stringify({
        xBRIEFInfo: { version: "0.8" },
        plan: {
  ...
  it("preserves explicit spec-render opt-ins for scopes and legacy artifacts (#1566)", () => {
    const root = mkdtempSync(join(tmpdir(), "deft-cli-spec-render-full-"));
    temps.push(root);
    const xbrief = join(root, "xbrief");
    mkdirSync(join(xbrief, "completed"), { recursive: true });
    writeFileSync(
    ...
    const result = runDeftTs("spec-render", [
      "--project-root",
      root,
      "--include-scopes=on",
      "--include-legacy-artifacts=on",
    ]);
    expect(result.exitCode).toBe(0);
    const content = readFileSync(join(root, "SPECIFICATION.md"), "utf8");
...
describe("deft-ts prd-render", () => {
  ...
  it("keeps PRD render compact by default and preserves explicit legacy opt-in (#1566)", () => {
    const root = mkdtempSync(join(tmpdir(), "deft-cli-prd-legacy-"));
    temps.push(root);
    mkdirSync(join(root, "xbrief"), { recursive: true });
    writeFileSync(
    ...
    const outPath = join(root, "PRD.md");
    const compact = runDeftTs("prd-render", ["--project-root", root, "--output", outPath]);
    ...
    const full = runDeftTs("prd-render", [
      "--project-root",
      root,
      "--output",
      outPath,
