...
| [deft-directive-review-cycle](./content/skills/deft-directive-review-cycle/SKILL.md) | Greptile / bot reviewer response loop: fetch findings, batch-fix all P0/P1, re-review until clean. **Canonical review surface** — any generic "review" request routes here by intent; host review tools (Cursor `bugbot` / `security-review` subagent types, `review-bugbot` / `review-security` skills, or any future host equivalent) are advisory-only inputs folded into this cycle, never a replacement (#2308). | `review cycle`, `check reviews`, `run review cycle`, `review`, `review this`, `get this reviewed`, `use sub-agents for reviews`, `code review`, `bugbot`, `security review` |
