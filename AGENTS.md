...
! When a maintainer-side rule lands in this `AGENTS.md` that is consumer-relevant (welcome / WIP cap / triage / install integrity / branch policy / encoding gates / canonical commands / skill routing), the same PR MUST update `content/templates/agents-entry.md` to mirror it, then run `task agents:refresh` so consumer-side AGENTS.md inherits the change. The deterministic gate `packages/core/src/content-contracts/skills/agents_entry_contract.test.ts` enforces this through whitespace-normalized command/header markers plus pointer-sufficient rule contracts: relocated rule bulk may be represented by a verified skill, gate, or doc pointer instead of full mirrored prose. Adding a new consumer-relevant rule means adding either the terse always-on marker or the typed pointer contract in the same PR.
