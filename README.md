...
>
> **New clone, or `deft` / `directive` won't run, or PATH version is behind the pin?** Directive runs from an npm-installed engine — there is no committed shell shim, so *you* (agent or human) are the interpreter. Work the **global-first ladder** below top-down and stop at the first rung that yields a working `directive` whose PATH version matches the pin. The version to install is always the one pinned in the project's committed `package.json`. Doctor exit 0 is not skip.
>
